...
public class InitDB {
    ...
    public static class InitService {
        ...
        public void initMember() {
            ...
            Member member2 = Member.builder().loginId("abcde2").password("abcdefg1!").role(Role.ROLE_USER)
                    .nickname("이태엽").build();


            Member member3 = Member.builder().loginId("abcde3").password("abcdefg1!").role(Role.ROLE_USER)
                    .nickname("하윤지").build();

            Member member4 = Member.builder().loginId("abcde4").password("abcdefg1!").role(Role.ROLE_USER)
                    .nickname("고봉훈").build();
            ...
            Waste waste = makeWaste("경기도", "수원시 농업기술센터", WasteStatus.REFUSE, 1, member2);

            Waste waste1 = makeWaste("경기도", "수원시 농업기술센터", WasteStatus.PERMIT, 2, member2);
...
//            wasteRepository.save(makeWaste("경기도", "수원시 센터", WasteStatus.PERMIT, 10, member2));
//            wasteRepository.save(makeWaste("경기도", "수원시 센터", WasteStatus.REFUSE, 10, member2));
...
//            wasteRepository.save(makeWaste("부산광역시", "강서구 센터", WasteStatus.PERMIT, 10, member2));
        ...
        public Waste makeWaste(String state, String centerName, WasteStatus status, int day, Member member) {
            Waste waste = Waste.builder().state(state).centerName(centerName).status(status)
                    .requestDate(LocalDate.now().plusDays(day))
                    .member(member).build();
            return waste;
        }
